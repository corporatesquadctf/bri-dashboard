...
abstract class REST_Controller extends CI_Controller {
    ...
    public function checkTokenMobile()
    {
        $headers = $this->input->request_headers();
        $datenow = date('Y-m-d H:i:s');

        if (!empty($headers['device'])) {
            if ($headers['device'] === 'android') {
        ...
        if (!empty($token)) {
            ...
            if (count($data) > 0) {
                $expiredDate = strtotime('+' . $data[0]->Expires . ' days', strtotime($data[0]->CreatedDate));
