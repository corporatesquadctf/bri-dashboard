...
class ProsesKredit_model extends MY_Model {
    ...
    function getProsesKredit($data){
        ...
        $sql = "SELECT  t1.*, 
                        t2.CustomerName, t2.Address, t2.BusinessType, t2.Plafond, t2.JangkaWaktu,
                        t3.Name AS RMName,
    ...
    function getTotalProsesKredit($data){
        $whereClause = " WHERE t2.StatusId = 6";
    ...
    function getProsesKreditDocument(){
        $this->db->select("t1.*, t2.Status, t2.Description");
        $this->db->from("ProsesKreditDocument t1");
        $this->db->join("ProsesKreditDocumentStatus t2", "t1.ProsesKreditDocumentId = t2.ProsesKreditDocumentId", "left");
        $result = $this->db->get()->result();
        return $result;
    }
