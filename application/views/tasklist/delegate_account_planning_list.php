            ...
            <div class="x_panel container_header">
                <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item active" aria-current="page"><a href="<?=base_url('tasklist/delegate');?>">Delegate Account Planning</a></li>
                  </ol>
                </nav>
                <div class="x_title">
                    <div class="page_title">
                        <div class="pull-left">Delegate Account Planning</div>
                    </div>
                </div>
            </div>
        </div>
          ...
          <div class="x_panel">
            <div>
              ...
              <div class="x_content" <?= ($search_box) ? $search_box : 'style="display: none;"' ?>>
                  <form action="<?=base_url();?>tasklist/delegate/search" method="post" class="form-horizontal form-label-left" style="background-color: #f4f4f4; padding: 12px 12px 12px 12px; border: 1px solid #d0d0d0">
                  ...
                  <?php foreach ($results as $ap_id => $ap_row) : ?>
                    <div class="x_panel col-md-12 col-sm-12 col-xs-12" style="padding: 0px; border-radius: 4px;">
                      ...
                      <div class="collapse col-md-12 col-sm-12 col-xs-12" id="collapseExample<?= html_escape($ap_row['AccountPlanningId']); ?>" style="background: #E5F0FF; padding: 10px;">
                        <div class="card card-body">
                          ...
                          <div class="col-md-3 col-sm-3 col-xs-12 padding_con">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                            <form>                              
                              <button type="button" class="btn" style="font-size: 10px; line-height: 136.89%; letter-spacing: 0.15px; background:transparent; border: 1px solid #F58C38; box-sizing: border-box; border-radius: 2px; color: #F58C38; width: 210px; height: 45px;" onclick="delegate_ap('<?=$ap_row['AccountPlanningId']?>');">DELEGATE ACCOUNT PLANNING</button>
                              <button type="button" class="btn" style="font-size: 10px; line-height: 136.89%; letter-spacing: 0.15px; background: #F58C38; border-radius: 2px; color: #fff; width: 210px; height: 45px;" onclick="window.location.href='<?= base_url('tasklist/delegate/view/'.$ap_row['AccountPlanningId'].'/details'); ?>'">VIEW ACCOUNT PLANNING</button>
                            </form>
                            </div>
                          </div>
                          <div class="clearfix"></div>
                      ...
                      <div class="detail_button_con col-md-12 col-sm-12 col-xs-12">
                        <a class="detail_button" data-toggle="collapse" href="#collapseExample<?= html_escape($ap_row['AccountPlanningId']); ?>" aria-expanded="true" aria-controls="collapseExample<?= html_escape($ap_row['AccountPlanningId']); ?>">
...
<?php $this->load->view('tasklist/account_planning_list_rm.php'); ?>
...
<script type="text/javascript">
  ...
  function delegate_ap(AccountPlanningId) {
    $(document).ready(function() {
      $('.loaderImage').show();
      $('.modal-add-account_planning_delegate').modal('show');
      $('#old_owner').remove();
      $('#AccountPlanningOwnerId').remove();
      $('#AccountPlanningId').remove();

      var form = $('#delegateAPModalForm');
      $(form).append('<input type="hidden" id="AccountPlanningId" name="AccountPlanningId" value="'+AccountPlanningId+'" /> ');
      
      $.getJSON(base_url+'tasklist/delegate/getAccountPlanningOwner/' + AccountPlanningId, function (data){
        console.log(data.UserId);
        $(form).append('<input type="hidden" id="old_owner" name="old_owner" value="'+data.UserId+'" /> ');
        $(form).append('<input type="hidden" id="AccountPlanningOwnerId" name="AccountPlanningOwnerId" value="'+data.AccountPlanningOwnerId+'" /> ');
      })
      ...
      var w ='<table width="100%" id="table_new_owners" class="table table-condensed table-striped table-hover"><thead style="background-color: #FFFFFF; color: #218FD8;" ><tr class="modal_table_title"><td width="5%"><i class="material-icons" style="color: rgba(0, 0, 0, 0.54);">check</i></td><td width="40%">Personal Number</td><td width="55%">Name</td></tr></thead>';

      $.getJSON(base_url+'tasklist/delegate/getRMPerUnitKerja/' + AccountPlanningId, function (data){
        console.log(data);
        if(data.length > 0){
          w += '<tbody>';
          $.each(data, function(index, item){
              w += '<tr class="modal_table_list">';
              w += '<td><label><input id="new_owner" name="new_owner" value="' + item.UserId + '" type="radio" class="flat"></label></td></td>';
              w += '<td>' + item.UserId + '</td>';
              w += '<td>' + item.RMName + '</td>';
              w += '</tr>';
          })
