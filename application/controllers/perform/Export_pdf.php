...
class Export_pdf extends MY_Controller {
    ...
    public function index($customer_vcif, $year, $page = 1) {
...
SQL
        ...
        $data['groupoverview'] = array();

        $this->db->select(<<<SQL
                GROUP_OVERVIEWS.ADDRESS1,
                GROUP_OVERVIEWS.VCIF,
                GROUP_OVERVIEWS.CITY_ID,
                GROUP_OVERVIEWS.INDUSTRY_NAME,
                b.Description,
                c.NAME name_global,
                c.DESCRIPTION global_desc,
                d.NAME name_domestic,
                e.NAME name_industrytrend,
                f.NAME name_life,

SQL
        );
        $this->db->from('GROUP_OVERVIEWS');
        $this->db->join('LOOKUP_PROVINSI b', 'b.PKLookup = GROUP_OVERVIEWS.CITY_ID', 'left');
        $this->db->join('MASTER_GLOBALRATINGS c', 'c.ID = GROUP_OVERVIEWS.GLOBALRATING_ID', 'left');
        $this->db->join('MASTER_DOMESTICRATINGS d', 'd.ID = GROUP_OVERVIEWS.DOMESTICRATING_ID', 'left');
        $this->db->join('MASTER_INDUSTRYTRENDS e', 'e.ID = GROUP_OVERVIEWS.INDUSTRYTYPE_ID', 'left');
        $this->db->join('MASTER_LIFECYCLES f', 'f.ID = GROUP_OVERVIEWS.LIFECYCLE_ID', 'left');
        $this->db->where('VCIF', $customer_vcif);
        $groupoverview = $this->db->get()->row_array();
...
SQL
        ...
        $this->load->view('performance/export_pdf.php', $data);
    }
