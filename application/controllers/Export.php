<?php
    ...
    class Export extends CI_Controller {
        ...
        public function preview_account_planning($AccountPlanningId){
            if(empty($this->session->USER_ID)) redirect('logins');
            ...
            $this->load->view("export/account_planning", $data);
        }
        public function print_account_planning($AccountPlanningId){
            if(empty($this->session->USER_ID)) redirect('logins');
            $this->load->library("pdfgenerator");
            $data = array();
        ...
        public function surat_permohonan($prosesKreditId, $portofolio = 0){
            if(empty($this->session->USER_ID)) redirect('logins');
            $this->load->library("pdfgenerator");
            $data = array();
            ...
            }else if($portofolio == 1){
                $rsDetailProsesKredit = $this->PortofolioKredit_model->getDetailPortofolioKredit($prosesKreditId);
                $rsFasilitasPermohonan = $this->PortofolioKredit_model->getFasilitasPermohonan($prosesKreditId);
            }
            $totalPlafond = 0;
            if(!empty($rsFasilitasPermohonan)):
                foreach($rsFasilitasPermohonan as $row):
                    $totalPlafond += $row->Plafond;
                endforeach;
            endif;
            $terbilang = $this->terbilang($totalPlafond)." Rupiah";
            $rsDetailProsesKredit->PermohonanTerbilang = $terbilang;
        ...
        /* Export Dokumen LKN */
        public function dokumen_lkn($prosesKreditId, $portofolio = 0){
            if(empty($this->session->USER_ID)) redirect('logins');
            $this->load->library("pdfgenerator");
            $data = array();
        ...
        public function kelengkapan_dokumen($prosesKreditId, $portofolio = 0){
            if(empty($this->session->USER_ID)) redirect('logins');
            $this->load->library("pdfgenerator");
            $data = array();
            ...
            if($portofolio == 0){
                ...
                $rsADK = $this->ProsesKredit_model->getProsesKreditDocumentStatus($prosesKreditId);
                $data["Portofolio"] = 0;
            }else if($portofolio == 1){
                $rsDetailProsesKredit = $this->PortofolioKredit_model->getDetailPortofolioKredit($prosesKreditId);
                $rsProsesKreditDocument = $this->ProsesKredit_model->getProsesKreditDocument();
                foreach($rsProsesKreditDocument as $row){
                    $rsProsesKreditDocumentStatus = $this->PortofolioKredit_model->getPortofolioKreditDocumentStatus($prosesKreditId, $row->ProsesKreditDocumentId);
                    if(!empty($rsProsesKreditDocumentStatus)){
                        $row->Status = $rsProsesKreditDocumentStatus[0]->Status;
                        $row->Description = $rsProsesKreditDocumentStatus[0]->Description;
                    }else{
                ...
                $rsADK = $this->PortofolioKredit_model->getPortofolioKreditDocumentStatus($prosesKreditId);
                $data["Portofolio"] = 1;
            }

            /* Get ADK Information */
            if(!empty($rsADK)){
                $userIdAdk = $rsADK[0]->ModifiedBy;
                $rsADKInformation = $this->User_model->get_array_user_data($userIdAdk);
            }else{
                $rsADKInformation = array(
                    "Name" => "",
                    "RoleName" => "",
                    "UnitKerjaName" => ""
                );
            ...
            $data["ADKInformation"] = $rsADKInformation;
