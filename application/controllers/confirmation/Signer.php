...
class Signer extends MY_Controller {
	...
	public function index($rowno=0) {
	    ...
	    $params = array();
		$params['confirmation_user'] = 'Signer';
	    $limit_per_page = 5;
	    ...
	    $total_records = $this->ConfirmationAccountPlanning_model->getTotalViewSignerAccountPlanning($this->session->PERSONAL_NUMBER);
	    if ($total_records > 0) {   
	      	$ap_Tasklist = $this->ConfirmationAccountPlanning_model->getViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno);
			...
			$config['base_url'] = base_url() . 'confirmation/Signer/page';
	...
	public function search($searchTxt='', $rowno=0) {
	    ...
	    if(empty(trim($searchTxt))) {
	      header("location:".base_url() . "confirmation/Signer");
	    }
	    ...
	    $total_records = $this->ConfirmationAccountPlanning_model->getTotalViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $searchTxt);
	    if ($total_records > 0) {   
	      	$ap_Tasklist = $this->ConfirmationAccountPlanning_model->getViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno, $searchTxt);
			...
			$config['base_url'] = base_url() . 'confirmation/Signer/search/'.$params['searchTxt'];
			$config['use_page_numbers'] = TRUE;
			$config['total_rows'] = $total_records;
			$config['per_page'] = $limit_per_page;
	...
	public function add_CheckerSignerResponse() {
		if (!empty($this->input->post('confirmation_table_id')) && !empty($this->input->post('AccountPlanningId'))) {
			...
			$SignerDetail = $this->ConfirmationAccountPlanning_model->getSignerDetail($this->input->post('AccountPlanningId'), $this->session->PERSONAL_NUMBER);
	        ...
	        if ($account_planning_status['DocumentStatusId'] == 3 && empty($SignerDetail['IsApproved'])) {
				...
				$SignerList = $this->ConfirmationAccountPlanning_model->getSignerList($this->input->post('AccountPlanningId'));
				...
				if (!empty($data_insert)) {
					...
					if($DocumentStatusId == 6)
						$this->TasklistAccountPlanning_model->insertAccountPlanningActivity($this->input->post('AccountPlanningId'), 'Approval Signer', 'Rejected by signer: '.$this->input->post('Comment'), $_SESSION['PERSONAL_NUMBER']);
					else
						$this->TasklistAccountPlanning_model->insertAccountPlanningActivity($this->input->post('AccountPlanningId'), 'Approval Signer', 'Approved by signer: '.$this->input->post('Comment'), $_SESSION['PERSONAL_NUMBER']);
				}
			}
	        else {
                $result = array(
	...
	public function history($rowno=0) {
	    ...
	    $total_records = $this->ConfirmationAccountPlanning_model->getTotalViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $params['searchTxt'], '', '', '', 1);
	    if ($total_records > 0) {   
	      	$ap_Tasklist = $this->ConfirmationAccountPlanning_model->getViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno, $params['searchTxt'], '', '', '', 1);
			...
			$config['base_url'] = base_url() . 'confirmation/Signer/history';
			$config['use_page_numbers'] = TRUE;
			$config['total_rows'] = $total_records;
			$config['per_page'] = $limit_per_page;
			$config["uri_segment"] = 4;
	...
	public function history_search($searchTxt='', $rowno=0) {
	    ...
	    if(empty(trim($searchTxt))) {
	      header("location:".base_url() . "confirmation/Signer/history");
	    }
	    ...
	    $total_records = $this->ConfirmationAccountPlanning_model->getTotalViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $searchTxt, '', '', '', 1);
	    if ($total_records > 0) {   
	      	$ap_Tasklist = $this->ConfirmationAccountPlanning_model->getViewSignerAccountPlanning($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno, $searchTxt, '', '', '', 1);
			...
			$config['base_url'] = base_url() . 'confirmation/Signer/history/'.$params['searchTxt'];
			$config['use_page_numbers'] = TRUE;
			$config['total_rows'] = $total_records;
			$config['per_page'] = $limit_per_page;
			$config["uri_segment"] = 5;
