...
class AccountPlanningCst extends MY_Controller {
  ...
  public function index($rowno=0) {
    ...
    $total_records = $this->TasklistAccountPlanning_model->getTotalMyAccountPlanningCst($this->session->PERSONAL_NUMBER, $this->current_year);
    if ($total_records > 0) {   

      $ap_Tasklist = $this->TasklistAccountPlanning_model->getMyAccountPlanningCst($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno, $this->current_year);
      ...
      $config['base_url'] = base_url() . 'tasklist/AccountPlanningCst/page';
    ...
    $this->load->view('tasklist/account_planning_cst_list.php', $params);
  ...
  public function search($fYear='',$fDocStatus='',$fSearchTxt='',$rowno=0) {
    ...
    $total_records = $this->TasklistAccountPlanning_model->getTotalMyAccountPlanningCst($this->session->PERSONAL_NUMBER, $fYear, $fDocStatus, $fSearchTxt);
    $params['keyword_search_box'] = $fSearchTxt;
    if ($total_records > 0) {   

      $ap_Tasklist = $this->TasklistAccountPlanning_model->getMyAccountPlanningCst($this->session->PERSONAL_NUMBER, $limit_per_page, $rowno, $fYear, $fDocStatus, $fSearchTxt);
      ...
      $config['base_url'] = base_url() . 'tasklist/AccountPlanningCst/search/'.$fYear.'/'.$fDocStatus.'/'.$searchUrl;
      $config['use_page_numbers'] = TRUE;
      $config['total_rows'] = $total_records;
      $config['per_page'] = $limit_per_page;
  ...
  public function view($AccountPlanningId, $AccountPlanningTabType='details', $AccountPlanningTab='') {
    // $this->checkCST($AccountPlanningId, $AccountPlanningTab='', $AccountPlanningSubTab='');

    $data['AccountPlanningTabType1']               = 'details';
    $data['AccountPlanningTabType2']               = 'details';
    $data['AccountPlanningTabType3']               = 'details';
    $data['AccountPlanningTabType4']               = 'details';
    $data['AccountPlanningTabType5']               = 'details';

    $CSTPrivilegeTab = $this->PerformanceAccountPlanning_model->getCSTPrivilegeTab($AccountPlanningId, $this->session->PERSONAL_NUMBER);
    $CSTPrivilege = explode(";", $CSTPrivilegeTab['PrivilegeTab']);
    foreach ($CSTPrivilege as $key => $value) {
      $data['AccountPlanningTabType'.$value]             = 'input';
    }
    ...
    $this->load->view('layout/side-nav.php');
    $this->load->view('layout/top-nav.php');
    $this->load->view('tasklist/account_planning_cst_ajax.php', $data);
    $this->load->view('layout/footer.php');
  }
}
