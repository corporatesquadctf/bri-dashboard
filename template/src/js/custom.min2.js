...
function init_flot_chart() {
    if ("undefined" != typeof $.plot) {
        ...
        $("#chart_plot_01").length && (console.log("Plot1"), $.plot($("#chart_plot_01"), [a, b], g)),
                ...
                tooltipFillColor: "rgba(51, 51, 51, 0.55)", data: d, lines: {
        ...
        var a = {
            ...
            data: {
                labels: ["Target",
                    "Pencapaian"],
                datasets: [{
                        data: [45, 65], backgroundColor: ["#BDC3C7", "#9B59B6", "#E74C3C", "#26B99A", "#3498DB"], hoverBackgroundColor: ["#CFD4D8", "#B370CF", "#E95E4F", "#36CAAB", "#49A9EA"]
                    }
                ]
            }
            ,
...
function init_charts() {
    if (console.log("run_charts  typeof [" + typeof Chart + "]"),
        ...
        if ($("#feeIncome").length) {
            new Chart(document.getElementById("feeIncome"),
                    {
                        type: "line",
                        data: {
                            labels: ["January",
                                ...
                                "July"],
                            datasets: [{
                                    label: "x", backgroundColor: "rgba(38, 185, 154, 0.31)", borderColor: "rgba(38, 185, 154, 0.7)", pointBorderColor: "rgba(38, 185, 154, 0.7)", pointBackgroundColor: "rgba(38, 185, 154, 0.7)", pointHoverBackgroundColor: "#fff", pointHoverBorderColor: "rgba(220,220,220,1)", pointBorderWidth: 1, data: [31, 74, 6, 39, 20, 85, 7]
                                }
                                ,
                                {
                                    label: "My Second dataset", backgroundColor: "rgba(3, 88, 106, 0.3)", borderColor: "rgba(3, 88, 106, 0.70)", pointBorderColor: "rgba(3, 88, 106, 0.70)", pointBackgroundColor: "rgba(3, 88, 106, 0.70)", pointHoverBackgroundColor: "#fff", pointHoverBorderColor: "rgba(151,187,205,1)", pointBorderWidth: 1, data: [82, 23, 66, 9, 99, 4, 2]
        ...
        */
        ...
        if ($("#mybarChart3").length) {
            var f = document.getElementById("mybarChart3");
            new Chart(f,
                    {
                        type: "bar",
                        data: {
        ...
        if ($("#loanSector").length) {
            var f = document.getElementById("loanSector"),
                    i = {
                        labels: ["Textile",
                            "Transportasi",
                            "Energi",
                            ...
                            "Property"],
                        datasets: [{
                                data: [120, 50, 140, 180, 100, 30, 20, 90, 35], backgroundColor: ["#455C73", "#9B59B6", "#BDC3C7", "#26B99A", "#3498DB", "#455C73", "#9B59B6", "#26B99A", "#3498DB"], hoverBackgroundColor: ["#34495E", "#B370CF", "#CFD4D8", "#36CAAB", "#49A9EA"]
                            }
                        ]
                    }
            ;
        ...
        if ($("#segmnet").length) {
            var f = document.getElementById("segmnet"),
                    i = {
                        datasets: [{
                                data: [120, 50, 140, 180, 100], backgroundColor: ["#455C73", "#9B59B6", "#BDC3C7", "#26B99A", "#3498DB"], label: "My dataset"
                            }
        ...
        var e,
            ...
            select: function (a, b, c) {
                $("#fc_create").click(),
                    ...
                    var a = $("#title").val();
                    var desc = $("#descr").val();
                    var divisiSubmit = $("#divisi").val();
                    var url99 = $("#baseurl").val()
                    if (a != "") {
                        var url = url99 + 'add_calendar';
                        var success = function (result) {
                            //location.reload();
                        };
                        $.post(url, JSON.stringify({'tgl': moment(e).format('DD-MM-YYYY'), 'msg': a, 'desc': desc, 'divisi': divisiSubmit}), success);
                    }
                    return b && (ended = b), f = $("#event_type").val(), a && g.fullCalendar("renderEvent", {
                        title: a, desk: desc, divisi: divisiSubmit, start: e, allDay: c, editable: true
                    }
                    , !0), $("#title").val(""), $("#descr").val(""), $("#divisi").val("")
                            , g.fullCalendar("unselect"), g.fullCalendar('rerenderEvents'), $(".antoclose").click(), !1
                }
                )
            }
            ,
            eventClick: function (a, b, c) {
                if (a.editable) {
                    console.log(a),
                            ...
                            $("#title2").val(a.title),
                            $("#descr2").val(a.desk),
                            $("#divisi2").val(a.divisi),
                            $('#divisi2').prop('disabled', true),
                            $('#divisi2').trigger('chosen:updated'),
                            $("#id2").val(a.id),
                            f = $("#event_type").val(),
                        ...
                        var divisiSubmit = $("#divisi2").val();
                        var url99 = $("#baseurl").val();
                        var url = url99 + 'chg_calendar';
                        if (tit != "") {
                            console.log('Posting data calendar');
                            var success = function (result) {
                                //location.reload();
                            };
                            $.post(url, JSON.stringify({'id': thisId, 'tgl': moment(e).format('DD-MM-YYYY'), 'msg': tit, 'desc': descr, 'divisi': divisiSubmit}), success);
                            a.title = $("#title2").val(), a.id = $("#id2").val(), a.desk = $("#descr2").val(), a.divisi = $("#divisi2").val()
                            g.fullCalendar("updateEvent", a), $(".antoclose2").click(),
                                    g.fullCalendar("unselect"),
                                    g.fullCalendar('rerenderEvents')
                        }
                    }
                    ),
                            g.fullCalendar("unselect")
                    ...
                    $("#title3").val(a.title),
                            $("#descr3").val(a.desk),
                            $("#divisi3").val(a.division),
                            $('#divisi3').prop('disabled', true),
                            $('#divisi3').trigger('chosen:updated'),
                            f = $("#event_type").val(),
                            g.fullCalendar("unselect")
                }
            },
    ...
    if (console.log("run_datatables"),
        ...
        var a = function () {
            $("#datatable-buttons").length && $("#datatable-buttons").DataTable({
                dom: "Bfrtipl", //lengthMenu: [[10, 25, 50, -1], [10, 25, 50, "All"]],
                //buttons:[
                //        {extend:"copy", className:"btn-sm",exportOptions:{modifier:{order:'current',page:'all',search:'none'}}}, 
                //        {extend:"csv", className:"btn-sm",exportOptions:{modifier:{order:'current',page:'all',search:'none'}}}, 
                //        {extend:"excel", className:"btn-sm",exportOptions:{modifier:{order:'current',page:'all',search:'none'}}}, 
